package com.example.navigationsearchview;
...
//com.example.navigationsearchview.NavigationSearchView
...
import static com.example.navigationsearchview.SuggestionsAdapter.getColumnString;
 ...
 * {@link android.widget.SearchView} class.
 * </p>
 *
 * <p>
 * When the SearchView is used in an {@link android.support.v7.app.ActionBar} as
 * an action view, it's collapsed by default, so you must provide an icon for
 * the action.
 * </p>
 * <p>
 ...
 * href="{@docRoot}guide/topics/search/index.html">Search</a> API guide.
 * Additional information about action views is also available in the <<a
 * href="{@docRoot}guide/topics/ui/actionbar.html#ActionView">Action Bar</a> API
 * guide
 * </p>
 * </div>
 *
 ...
 */
public class NavigationSearchView extends LinearLayoutCompat implements
	...
	private SearchNavigationListener mSearchNavListener;
	...
	public NavigationSearchView(Context context) {
		this(context, context.getResources()
				.getLayout(R.layout.abc_search_view));
	}
	...
	public NavigationSearchView(Context context, AttributeSet attrs,
		...
		mSearchPlate.setBackground(a
				.getDrawable((R.styleable.SearchView_queryBackground)));
		mSubmitArea.setBackground(a
				.getDrawable((R.styleable.SearchView_submitBackground)));
		mSearchButton.setImageDrawable(a
				.getDrawable(R.styleable.SearchView_searchIcon));
		mGoButton
				.setImageDrawable(a.getDrawable(R.styleable.SearchView_goIcon));
		mCloseButton.setImageDrawable(a
				.getDrawable(R.styleable.SearchView_closeIcon));
		mVoiceButton.setImageDrawable(a
				.getDrawable(R.styleable.SearchView_voiceIcon));
		mCollapsedIcon.setImageDrawable(a
				.getDrawable(R.styleable.SearchView_searchIcon));
		...
		mSearchHintIcon = a
				.getDrawable(R.styleable.SearchView_iconifiedByDefault);

		// Extract dropdown layout resource IDs for later use.
		mSuggestionRowLayout = a.getResourceId(
				R.styleable.SearchView_suggestionRowLayout,
				R.layout.abc_search_dropdown_item_icons_2line);
		mSuggestionCommitIconResId = a.getResourceId(
				R.styleable.SearchView_commitIcon, 0);
					...
					public void onFocusChange(View v, boolean hasFocus) {
						if (mOnQueryTextFocusChangeListener != null) {
							mOnQueryTextFocusChangeListener.onFocusChange(
									NavigationSearchView.this, hasFocus);
						}
					}
				});
		setIconifiedByDefault(a.getBoolean(
				R.styleable.SearchView_iconifiedByDefault, true));

		final int maxWidth = a.getDimensionPixelSize(
				R.styleable.SearchView_android_maxWidth, -1);
		if (maxWidth != -1) {
			setMaxWidth(maxWidth);
		}

		final CharSequence queryHint = a
				.getText(R.styleable.SearchView_queryHint);
		if (!TextUtils.isEmpty(queryHint)) {
			setQueryHint(queryHint);
		}

		final int imeOptions = a.getInt(
				R.styleable.SearchView_android_imeOptions, -1);
		if (imeOptions != -1) {
			setImeOptions(imeOptions);
		}

		final int inputType = a.getInt(
				R.styleable.SearchView_android_inputType, -1);
		if (inputType != -1) {
	...
	public boolean requestFocus(int direction, Rect previouslyFocusedRect) {
		...
		if (!isIconified()) {
			boolean result = mSearchSrcTextView.requestFocus(direction,
	...
	private void updateViewsVisibility(final boolean collapsed) {
		...
		final boolean hasText = !TextUtils
				.isEmpty(mSearchSrcTextView.getText());
	...
	private void updateSubmitArea() {
		...
		if (isSubmitAreaEnabled()
				&& (mGoButton.getVisibility() == VISIBLE || mVoiceButton
	...
	private void updateCloseButton() {
		...
		if (closeButtonImg != null) {
			closeButtonImg.setState(hasText ? ENABLED_STATE_SET
					: EMPTY_STATE_SET);
		}
	}
	...
	View.OnKeyListener mTextKeyListener = new View.OnKeyListener() {
		public boolean onKey(View v, int keyCode, KeyEvent event) {
			...
			if (DBG) {
				Log.d(LOG_TAG,
						"mTextListener.onKey(" + keyCode + "," + event
								+ "), selection: "
				...
				if (event.getAction() == KeyEvent.ACTION_UP) {
					if (keyCode == KeyEvent.KEYCODE_ENTER) {
						...
						launchQuerySearch(KeyEvent.KEYCODE_UNKNOWN, null,
								mSearchSrcTextView.getText().toString());
						return true;
					}
				}
			}
		...
		if (event.getAction() == KeyEvent.ACTION_DOWN
			...
			if (keyCode == KeyEvent.KEYCODE_DPAD_LEFT
				...
				HIDDEN_METHOD_INVOKER
						.ensureImeVisible(mSearchSrcTextView, true);
			...
			if (keyCode == KeyEvent.KEYCODE_DPAD_UP
					&& 0 == mSearchSrcTextView.getListSelection()) {
	...
	private CharSequence getDecoratedHint(CharSequence hintText) {
		...
		ssb.setSpan(new ImageSpan(mSearchHintIcon), 1, 2,
	...
	private void adjustDropDownSizeAndPosition() {
		if (mDropDownAnchor.getWidth() > 1) {
			...
			int iconOffset = mIconifiedByDefault ? res
					.getDimensionPixelSize(R.dimen.abc_dropdownitem_icon_width)
					+ res.getDimensionPixelSize(R.dimen.abc_dropdownitem_text_padding_left)
					: 0;
			mSearchSrcTextView.getDropDownBackground().getPadding(
					dropDownPadding);
			int offset;
			if (isLayoutRtl) {
	...
	private final OnItemSelectedListener mOnItemSelectedListener = new OnItemSelectedListener() {
		...
		public void onItemSelected(AdapterView<?> parent, View view,
			...
			NavigationSearchView.this.onItemSelected(position);
		}
	...
	private void setQuery(CharSequence query) {
		...
		mSearchSrcTextView.setSelection(TextUtils.isEmpty(query) ? 0 : query
				.length());
	}
	...
	public SearchNavigationListener getSearchNavListener() {
		return mSearchNavListener;
	}

	public void setSearchNavListener(SearchNavigationListener listnr) {
		mSearchNavListener = listnr;
	}
	...
	private TextWatcher mTextWatcher = new TextWatcher() {
		...
		public void onTextChanged(CharSequence s, int start, int before,
				int after) {
			NavigationSearchView.this.onTextChanged(s);
		}
	...
	public static class SearchAutoComplete extends AutoCompleteTextView {
		...
		private NavigationSearchView mSearchView;
		...
		void setSearchView(NavigationSearchView searchView) {
	...
	private static class AutoCompleteTextViewReflector {
		...
		AutoCompleteTextViewReflector() {
			...
			try {
				ensureImeVisible = AutoCompleteTextView.class.getMethod(
			...
			try {
				showSoftInputUnchecked = InputMethodManager.class.getMethod(
						"showSoftInputUnchecked", int.class,
